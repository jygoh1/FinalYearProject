...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    for t=1:nr
        ...
        xih1_temp = 10^(qq.asnr/10)*mean(mm); % speech-present SNR
        xih1r_temp = 1/(1 + xih1_temp)-1;
        
%         pfac=(1/(qq.pspri*mean(mm))-1)*(1+xih1_temp);
%         pfac=(1/(qq.pspri*mean(mm))-1)*(1+xih1);
%         pfac=(1/(qq.pspri)-1)*(1+xih1);
        pfac=(1/(qq.pspri)-1)*(1+xih1_temp);
        
%         ph1y=(1+pfac*exp(xih1r*yft./xt)).^(-1); % a-posteriori speech presence prob (18)
        ph1y=(1+pfac*exp(xih1r_temp*yft./xt)).^(-1); % a-posteriori speech presence prob (18)
