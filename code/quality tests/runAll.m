function [SNRMDKFmask, SNRLPCmask, SNRMDKF, SNRmmse, SNRbabble, pesqMDKF, pesqMDKFmask, pesqLPCmask, pesqMMSE, pesqNoisy] = runAll(y_clean, y_babble, fs)
...
% y_MDKF_clean = y_MDKF_clean(1:cutoff);
% y_MDKF_uncorrIBM_clean = y_MDKF_uncorrIBM_clean(1:cutoff);
% y_maskLPC_clean = y_maskLPC_clean(1:cutoff);
% 
% MDKF_clean = snrseg(y_MDKF_clean, y_clean, fs);
% MDKF_uncorrIBM_clean = snrseg(y_MDKF_uncorrIBM_clean, y_clean, fs);
% maskLPC_clean = snrseg(y_maskLPC_clean, y_clean, fs);
% 
% outputSNR = [MDKF_uncorrIBM, maskLPC, MDKF, mmse, babble, MDKF_uncorrIBM_clean, maskLPC_clean, MDKF_clean];

% outputSNR = [SNRMDKFmask, SNRLPCmask, SNRMDKF, SNRmmse, SNRbabble];
...
% outputPESQ = [pesqMDKF, pesqMDKFmask, pesqLPCmask, pesqNoisy];
